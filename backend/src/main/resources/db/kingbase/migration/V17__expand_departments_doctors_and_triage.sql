...
SELECT d.id, dept.id, (CURRENT_DATE + days.n), period.p, period.slots,
       GREATEST(0, period.slots - CAST(FLOOR(RANDOM() * 20) AS INTEGER)), 'NORMAL', 'ACTIVE', 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM (
...
WHERE NOT EXISTS (
    ...
    WHERE s.doctor_id = d.id
      AND s.work_date = (CURRENT_DATE + days.n)
